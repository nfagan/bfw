...
%     intermediate directory `INTERMEDIATE`. The full path to
%     `INTERMEDIATE` is generated from the saved config file.
%
%     X = load_one_intermediate( ..., NAME ) loads one file that contains
%     the char vector `NAME`. If no files contain `NAME`, `X` is an empty 
%     array.
%
