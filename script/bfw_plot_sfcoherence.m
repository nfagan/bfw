...
sfcoh_types = { '1channel_eyes', '1channel_mouth', '1channel_object' };
...
bfw.unify_nonsocial_object_labels( labels );
...
pairs = { {'mutual', 'm1'}, {'eyes_nf', 'mouth'} ...
  , {'eyes_nf', 'nonsocial_object'}, {'eyes_nf', 'everywhere'} };

sub_coh = [];
sub_labels = fcat();

for i = 1:numel(pairs)
  shared_utils.general.progress( i, numel(pairs) );
...
is_subtracted = true;
is_subtract_roi = true;

base_prefix = '';
base_subdir = 'eyes_minus_xx';
...
loop_I = findall_or_one( plt_labels, loop_cats, mask );

for idx = 1:numel(loop_I)

  pl = plotlabeled.make_spectrogram( freqs(f_ind), t(t_ind) );
  pl.sort_combinations = true;

  fig_I = findall( plt_labels, fcats, loop_I{idx} );
