...
%%  Unit meta data for cells that significnatly discriminate reward size

% method_subdir = '3_reward_levels_glm';
method_subdir = 'small_vs_large';

sig_reward_info = bfw_lda.load_sig_reward_level_unit_info( ....
  {'092719', '5_trials_per_condition', 'cs_target_acquire_cs_delay_cs_reward' ...
  , '_iti_baseline_norm_-250_0__0_250__50_600', method_subdir} );

sig_gaze_labels = bfw_ct.significant_unit_info_to_fcat( sig_reward_info );
...
%%  train reward test gaze

is_significant_units = false;
match_num_cells_to_significant_num_cells = false;

t_windows = { [-0.25, 0], [0, 0.25], [0.05, 0.6] };
kind = sprintf( 'train_reward_test_gaze-%s', method_subdir );
...
if ( is_significant_units )
  ...
  sig_str = 'significant-reward';
else
  find_unit_func = @bfw_ct.findnot_significant_unit_info;
  sig_str = 'nonsignificant-reward';
  
  if ( match_num_cells_to_significant_num_cells )
    additional_funcs = {
