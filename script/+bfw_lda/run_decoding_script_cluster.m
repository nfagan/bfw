...
sig_either_mask_func = make_significant_either_gaze_reward_mask_func( conf );
...
require_sig_reward = [ true ];
require_sig_gaze = [ true ];
...
for idx = 1:size(cmbs, 2)
  ...
  is_train_reward = ~isempty( strfind(kind, 'train_reward') );
  
%   if ( ~use_sig_for_train_only && ~is_sig_reward && ~is_sig_gaze )
...
%   end
  ...
  for i = 1:numel(t_windows)
    ...
    if ( use_sig_for_train_only )
      base_subdir = sprintf( '%s-train-sig-reward-sig-gaze', base_subdir );
      
      if ( is_train_reward )
        reward_mask_func = sig_either_mask_func;
        gaze_mask_func = base_gaze_mask_func;
      else
        reward_mask_func = base_reward_mask_func;
        gaze_mask_func = @(labels, mask) sig_either_mask_func(labels, base_gaze_mask_func(labels, mask));
      end
    else
      if ( is_sig_reward )
...
function func = make_significant_either_gaze_reward_mask_func(conf)

reward_func = make_significant_reward_mask_func( conf );
gaze_func = make_significant_gaze_mask_func( conf );

func = @(labels, mask) union(reward_func(labels, mask), gaze_func(labels, mask));
