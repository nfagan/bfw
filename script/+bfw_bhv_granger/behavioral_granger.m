...
parfor i = 1:numel(each_I)  
  ...
  if ( isempty(m1_ind) || isempty(m2_ind) )
    ...
    for j = 1:numel(bin_inds)
      ...
      [granger_f(1, j), granger_cv(1, j)] = ...
        granger_cause( smooth_m1, smooth_m2, alpha, max_lag );
      % reverse direction
      [granger_f(2, j), granger_cv(2, j)] = ...
        granger_cause( smooth_m2, smooth_m1, alpha, max_lag );
