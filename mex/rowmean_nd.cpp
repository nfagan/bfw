...
namespace util {  
  ...
  void run_threaded(const DecomposedArray<double> &input_data,
    ...
    int status = util::run(input_data, indices, start, stop, 
    ...
    if (use_threads) {      
      ...
      for (int64_t i = 0; i < n_threads; i++) {
        threads.emplace_back([&, i]() -> void {
          run_threaded(input_data, indices, dimension_index_combinations, 
      ...
      return run(input_data, indices, 0, n_indices,
                 dimension_index_combinations, out_data_ptr);
    }
  }
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  mxArray *output_array = make_output_array(indices.size(), input_data.descriptor);
  ...
  int status = run(input_data, indices, dim_index_combs, out_data_ptr);
